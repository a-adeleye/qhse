...
@Component({
  ...
  templateUrl: './inspection-logs.html',
  styleUrl: './inspection-logs.scss'
})
export class InspectionLogs implements OnInit {
  @Input() listName = '';
  ...
  async loadLogs() {
    ...
    if (this.isOnline) {
      this.sharepointService.getListItems(this.listName).subscribe({
        next: (res) => {
  ...
  open(row: any) {
    this.router.navigate([`/logs/${row.listName}`, row.ID]).then();
  }
  ...
  private getLocalSubmissions(): any[] {
    ...
    return submissions.filter(
      (s: any) => s.listName === this.listName
    );
  }
  ...
  private submitFormData(formData: any): Promise<any> {
    ...
    delete formData.listName;
    return new Promise((resolve, reject) => {
      this.sharepointService.addListItem(listName, formData)
        .subscribe({
          next: (res) => {
